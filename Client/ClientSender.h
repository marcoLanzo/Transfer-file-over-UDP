//
//Header file per le operazioni di invio affidabile al server.
//
    ...
    for(;;){
        ...
        if(diff >= snd->t.TIMEOUT){ 		//se timeout scaduto rinvio il pacchetto
            ...
            if(ran < (1 - P)) {
                ...
                free(data_send);
                printf("Scaduto il timer pacchetto %d RE-inviato!\n", pkt->seqnum);
...
void proceduraFinale(struct snd_thread_info *snd_ti, struct snd_pack *pkt, char* ACK_torcv) {
    ...
    if(pthread_cancel(snd_ti->timeoutManager) != 0){	//cancello thread timeoutManager
        //perror("Errore in pthread_cancel()");
        exit(EXIT_FAILURE);
    }
...
void thread_ack_job(void* p)
{
    ...
    for(;;){
        ...
        printf("ackManager: send_base:%d, next_tosend:%d\n",snd_ti->send_base, snd_ti->next_tosend);
...
void sendFile(int sockfd,int fd,struct sockaddr_in si_other)
{
    ...
    if(pthread_create(&tid , NULL, thread_send_job, snd) != 0){
        perror("Errore in pthread_create() che si occupa di eseguire la send");
        exit(EXIT_FAILURE);
    }

    if(pthread_create(&tid, NULL, thread_timeout_job, snd) != 0){
        perror("Errore in pthread_create() che si occupa del timeout");
        exit(EXIT_FAILURE);
    }
